...
exports.run = (client, message, params) => {
  if (!params[0]) {
    ...
    message.channel.send(`= Lista komend =\n\n[Wpisz komendę ${config.prefix}help (nazwa komendy) dla szczegółów wybranej komendy. Komendy muzyczne są dostępne w komendzie cz!musichelp. Wersja bota: 1.0.2]\n\n${client.commands.map(c => `${config.prefix}${c.help.name}${' '.repeat(longest - c.help.name.length)} :: ${c.help.description}`).join('\n')}`, {code: 'asciidoc'});
  } else {
